...
describe("forecast service", () => {
  ...
  const forecastServiceCityInvalidMockedResponse = {
    cod: "404",
  ...
  test("should throw an error for invalid city name", async () => {
    fetchMock.mockResponseOnce(
      JSON.stringify(forecastServiceCityInvalidMockedResponse),
    )
    const weatherApiResponse = await getForecastByCity(invalidCity)
    expect(weatherApiResponse).toEqual(forecastServiceCityInvalidMockedResponse)
  })
  test("should throw an error when city is empty", async () => {
    await expect(() => getForecastByCity("")).rejects.toThrowError(
      ERROR_INVALID_CITY,
    )
  })

  test("should throw an error for invalid lon & lat", async () => {
    await expect(() => getForecastByLatLon(0, 0)).rejects.toThrowError(
      ERROR_INVALID_LAT_LON,
    )
  })

  test("should return forecast data based on city name", async () => {
    fetchMock.mockResponseOnce(JSON.stringify(forecastServiceMockedResponse))
    const response = await getForecastByCity(city)
    expect(response).toEqual(forecastServiceMockedResponse)
  })
  test("should return forecast data based on lon & lat", async () => {
    fetchMock.mockResponseOnce(JSON.stringify(forecastServiceMockedResponse))
    const response = await getForecastByLatLon(latitude, longitude)
    expect(response).toEqual(forecastServiceMockedResponse)
  })
})
